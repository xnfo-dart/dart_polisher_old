...
class HttpRequest extends $pb.GeneratedMessage {
    static final $pb.BuilderInfo _i = $pb
        .BuilderInfo(
            const $core.bool.fromEnvironment('protobuf.omit_message_names')
                ? ''
                : 'HttpRequest',
...
class BacktrackingSolver {
    ...
    BacktrackingSolver(SolveType type, SourceRegistry sources, this.root, this.lockFile,
    ...
    Future<SolveResult> solve() {
        ...
        return newFuture(() {
            ...
            return new SolveResult.success(sources, root, lockFile, packages, overrides,
                pubspecs, _getAvailableVersions(packages), attemptedSolutions);
        }).catchError((error) {
            if (error is! SolveFailure) throw error;
...
class Traverser {
    ...
    /// traversal is complete. Completes it to an error if a failure occurred.
    /// Otherwise, recurses.
    Future<List<PackageId>> _traverseDeps(PackageId depender, DependencyQueue deps) {
        // Move onto the next package if we've traversed all of these references.
        if (deps.isEmpty) return _traversePackage();
    ...
    Future _addImplicitDependencies() {
        ...
        return Future.wait(barback.pubConstraints.keys.map((depName) {
            var constraint = barback.pubConstraints[depName];
            _solver.logSolve('add implicit $constraint pub dependency on ' '$depName');
    ...
    PackageId _getValidLocked(String name) {
        ...
        if (required != null) {
            ...
            var source = _solver.sources[package.source];
            if (!source.descriptionsEqual(package.description, required.dep.description))
                return null;
        }
    ...
    Future _doCompilation(Transform transform) {
        ...
        return dart.compile(entrypoint, provider,
            ...
            checked: _configBool('checked'),
            minify:
                _configBool('minify', defaultsTo: _settings.mode == BarbackMode.RELEASE),
            verbose: _configBool('verbose'),
            environment: _configEnvironment,
            packageRoot: _environment.rootPackage.path("packages"),
            analyzeAll: _configBool('analyzeAll'),
