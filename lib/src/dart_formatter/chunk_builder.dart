...
class ChunkBuilder
{
    ...
    //! CHANGED(tekert) moved default value to initializer list of constructor,
    // removed private status, added @protected
    /// The current state.
    final NestingBuilder _nesting;
    ...
    ChunkBuilder(this._formatter, this._source)
        : _parent = null,
          _chunks = [],
          _nesting = NestingBuilder(_formatter)
    {
        indent(_formatter.indent);
        startBlockArgumentNesting();
    }

    //! CHANGED(tekert) added _nesting = NestingBuilder(_formatter) to initializer
        ...
        {Chunk? argumentChunk, bool indent = true, bool space = false})
    {
        ...
        var chunk = BlockChunk(
            argumentChunk, _rules.last, _nesting.indentation, _blockArgumentNesting.last,
    ...
    void _emitPendingWhitespace({bool isDouble = false, bool mergeEmptySplits = true})
    {
        ...
        _writeSplit(
            isDouble: isDouble, nest: _pendingNested, mergeEmptySplits: mergeEmptySplits);
    }
