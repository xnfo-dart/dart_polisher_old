...
import '../chunk.dart';
//import '../constants.dart'; //NOTE: indents are now taken from user options.
...
class SolveState
{
    ...
    //! CHANGED(tekert) added formatter for idents
    SolveState(this._splitter, this._ruleValues, this._formatter)
    {
        _calculateSplits();
        _calculateCost();
    }
        ...
        for (var rule in _splitter.rules)
        {
            if (_liveRules.contains(rule))
            {
                ...
                for (var value = 1; value < rule.numValues; value++)
                {
                    ...
                    //! CHANGED(tekert) added formatter for idents
                    var state = SolveState(_splitter, boundRules, _formatter);
            ...
            if (chunk.rule.isSplit(getValue(chunk.rule), chunk))
            {
                ...
                if (!chunk.flushLeft)
                {
                    ...
                    if (_splitter.chunks[i].indentBlock(getValue))
                    {
                        ...
                        indent += _formatter.options.tabSizes.expression;
