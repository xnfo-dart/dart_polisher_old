...
void main()
{
    ...
    test('formats a directory', () async
    {
        ...
        await expectLater(process.stdout, emits(startsWith('Formatting directory')));
    ...
    test('Overwrites files', () async
    {
        ...
        await expectLater(process.stdout, emits('Unchanged ${p.join('code', 'b.dart')}'));
        await expectLater(process.stdout, emits('Formatted ${p.join('code', 'c.dart')}'));
        await process.shouldExit(0);
    ...
    test('only prints a hidden directory once', () async
    {
        ...
        expect(
            await process.stdout.next, "Skipping hidden path ${p.join("code", ".skip")}");
    ...
    group('--set-exit-if-changed', ()
    {
        ...
        test('gives exit code 1 if there are changes even in dry run', () async
        {
            ...
            var process = await runFormatterOnDir(['--set-exit-if-changed', '--dry-run']);
            await process.shouldExit(1);
        });
    });
    ...
    group('fix', ()
    {
        ...
        {
            var process = await runFormatter(['--fix', '--fix-named-default-separator']);
            await process.shouldExit(64);
        });
    });
