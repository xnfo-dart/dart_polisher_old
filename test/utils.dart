...
Future<String> _compileSnapshot(String script) async
{
    ...
    var tempDir = await Directory.systemTemp.createTemp(p.withoutExtension(scriptName));
    ...
    var compileResult = await Process.run(Platform.resolvedExecutable,
        ['--snapshot-kind=app-jit', '--snapshot=$snapshot', scriptPath, '--help']);
...
void testDirectory(String name, [Iterable<StyleFix>? fixes])
{
    ...
    var entries =
        Directory(p.join(testDir, name)).listSync(recursive: true, followLinks: false);
