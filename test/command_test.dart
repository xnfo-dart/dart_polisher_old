...
void main()
{
    ...
    test('formats a directory', () async
    {
        ...
        expect(await process.stdout.next, startsWith(r'Formatted 3 files (2 changed)'));
    ...
    test('formats multiple paths', () async
    {
        ...
        expect(
            await process.stdout.next, 'Formatted ${p.join('code', 'subdir', 'a.dart')}');
        expect(await process.stdout.next, 'Formatted ${p.join('code', 'c.dart')}');
        expect(await process.stdout.next, startsWith(r'Formatted 2 files (2 changed)'));
        await process.shouldExit(0);
    ...
    group('--show', ()
    {
        test('all shows all files', () async
        {
            ...
            expect(await process.stdout.next, 'Formatted ${p.join('code', 'a.dart')}');
            expect(await process.stdout.next, 'Unchanged ${p.join('code', 'b.dart')}');
            expect(await process.stdout.next, 'Formatted ${p.join('code', 'c.dart')}');
            expect(
                await process.stdout.next, startsWith(r'Formatted 3 files (2 changed)'));
            await process.shouldExit(0);
        });
    ...
    group('--output', ()
    {
        group('show', ()
        {
            ...
            test('with --show=all prints all files and names first', () async
            {
                ...
                expect(await process.stdout.next, 'Changed ${p.join('code', 'a.dart')}');
                expect(await process.stdout.next, formattedOutput);
                expect(
            ...
            test('with --show=changed prints only changed files', () async
            {
                ...
                var process = await runCommandOnDir(['--output=show', '--show=changed']);
                expect(await process.stdout.next, 'Changed ${p.join('code', 'a.dart')}');
                expect(await process.stdout.next, formattedOutput);
                expect(await process.stdout.next,
                    startsWith(r'Formatted 2 files (1 changed)'));
        ...
        group('json', ()
        {
            ...
            {
                var process = await runCommandOnDir(['--output=json', '--summary=line']);
                await process.shouldExit(64);
            });
        });
        ...
        group('none', ()
        {
            ...
            test('with --show=changed prints only changed names', () async
            {
                ...
                var process = await runCommandOnDir(['--output=none', '--show=changed']);
                expect(await process.stdout.next, 'Changed ${p.join('code', 'a.dart')}');
                expect(await process.stdout.next,
                    startsWith(r'Formatted 2 files (1 changed)'));
                await process.shouldExit(0);
    ...
    group('fix', ()
    {
        ...
        {
            var process = await runCommand(['--fix', '--fix-named-default-separator']);
            await process.shouldExit(64);
        });
    });
    ...
    group('--set-exit-if-changed', ()
    {
        ...
        test('gives exit code 1 if there are changes when not writing', () async
        {
            ...
            var process = await runCommandOnDir(['--set-exit-if-changed', '--show=none']);
            await process.shouldExit(1);
        });
    });
